 ...
 */
const getEventByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Event,req.body);
    if (result){
      return res.ok({ data :result });
    }
    return res.recordNotFound();
...
const softDeleteManyEvent = async (req,res) => {
  try {
    ...
    let result = await deleteDependentService.softDeleteEvent(query, updateBody);
    if (!result) {
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const deleteManyEvent = async (req, res) => {
  try {
    ...
    if (req.body.isWarning) {
      let result = await deleteDependentService.countEvent(query);
      return res.ok({ data :result }); 
    }
    else {
      let result = await deleteDependentService.deleteEvent(query);
      return res.ok({ data :result });
    }
  } catch (error){
    return res.failureResponse({ data:error.message }); 
