 ...
 */
const getCommentByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Comment,req.body);
    if (result){
      return res.ok({ data :result });
    }
    return res.recordNotFound();
...
const softDeleteManyComment = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Comment,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
...
const deleteManyComment = async (req, res) => {
  try {
    ...
    const query = { '_id':{ '$in':ids } };
    let result = await dbService.deleteMany(Comment,query);
    return res.ok({ data :result });
  } catch (error){
    return res.failureResponse(); 
  }
...
const softDeleteComment = async (req,res) => {
  try {
    ...
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Comment, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const deleteComment = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Comment, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
