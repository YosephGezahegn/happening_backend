...
beforeAll(async function (){
  try {
    ...
    const db = client.db('Happening_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Jaquan_Gottlieb',
      password: 'sutPzo_pfMZQgNV',
      email: 'Vivianne_Barrows@hotmail.com',
      name: 'Nicole Sauer',
      organizer: false,
      admin: false,
      savedEvents: [ {
        eventSaved: '61f4bac64d9b4ab72d8b9a29',
        _id: false 
      } ],
      mobileNo: '779-764-6848 x410',
      role: 972,
      resetPasswordLink: {},
      loginRetryLimit: 640,
      loginReactiveTime: '2022-07-06T08:18:06.746Z',
      sso_auth: {
        googleId: 'backing',
        facebookId: 'connecting' 
      },
      id: '61f4bac64d9b4ab72d8b9a2a'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Christian26',
        'password':'VQyuGdrGdn5aPUs',
        'email':'Al_Pacocha@hotmail.com',
        'name':'Delbert Windler',
        'organizer':false,
        'admin':true,
        'savedEvents':[{
          'eventSaved':'61f4bac64d9b4ab72d8b9a18',
          '_id':false
        }],
        'mobileNo':'1-864-368-1792 x30548',
        'sso_auth':{
          'googleId':'calculating',
          'facebookId':'Frozen'
        },
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Al_Pacocha@hotmail.com', });
