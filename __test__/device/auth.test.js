...
beforeAll(async function (){
  try {
    ...
    inserted_user = await user.insertOne({
      username: 'Bennett31',
      password: 'yqWU6JecWJL92Fi',
      email: 'Eleanora6@hotmail.com',
      name: 'Emanuel Zemlak DDS',
      organizer: true,
      admin: false,
      savedEvents: [ {
        eventSaved: '61f4bac64d9b4ab72d8b9a5c',
        _id: false 
      } ],
      mobileNo: '(635) 472-1151 x90868',
      role: 522,
      resetPasswordLink: {},
      loginRetryLimit: 209,
      loginReactiveTime: '2022-04-09T05:54:25.607Z',
      sso_auth: {
        googleId: 'one-to-one',
        facebookId: 'Shoes' 
      },
      id: '61f4bac64d9b4ab72d8b9a5d'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Jimmy67',
        'password':'yA2TBQgWNLHbZSg',
        'email':'Cathrine.Kub@hotmail.com',
        'name':'Larry Rogahn',
        'organizer':true,
        'admin':true,
        'savedEvents':[{
          'eventSaved':'61f4bac64d9b4ab72d8b9a3a',
          '_id':false
        }],
        'mobileNo':'1-380-788-9625 x7526',
        'sso_auth':{
          'googleId':'generate',
          'facebookId':'Account'
        },
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/device/auth/forgot-password')
      .send({ 'email':'Cathrine.Kub@hotmail.com', });
